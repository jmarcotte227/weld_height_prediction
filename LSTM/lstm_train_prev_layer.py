...
def train(train_dataset, valid_dataset):
    ...
    for epoch in pbar:
        ...
        if valid_loss<best_loss:
            torch.save(model, f'saved_model_prev_layer.pt')
...
class WeldDataset(Dataset):
    ...
    def __init__(self,filepath, mean=None, std=None, norm=True):
        ...
        self.trg = torch.Tensor(data[1:,1:,-1])
        # 2 for number of inputs. to the network
        # 1 is the vel set
        # 2 is the previous segment dh
        # 3 is the previous layer dh
        # -1 on the first index (layer) to account for the previous layer data
        self.src = np.zeros((data.shape[0]-1,data.shape[1]-1,3))

        # right shift the set point
        self.src[:,:,0] = data[1:,1:,0]
        # previous height measurements
        self.src[:,:,1] = data[1:,:-1,-1]
        # previous temp measurements 
        # flip the previous layer and the height as the input
        self.src[:,:,1] = np.flip(data[:-1,:-1,-1], axis=1)
...
def test(valid_dataset, nonorm_dataset):
    # load model
    model = torch.load('saved_model_prev_layer.pt')
    # load data
    ...
    print('-----------------')
    print(f'LSTM MAE:     {mae(errors)}')
    print(f'LSTM RMSE:    {rmse(errors)}')
    print(f'LSTM max:     {max(errors)}')
